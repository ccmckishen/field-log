...
with tab2:
    if "user" not in st.session_state:
        ...
        try:
            response = supabase.table("field_logs").select("*").eq("user_id", st.session_state["user"].id).order("timestamp", desc=True).execute()
...
with tab3:
    ...
    if "user" in st.session_state:
        try:
            response = supabase.table("field_logs").select("*").eq("user_id", st.session_state["user"].id).execute()
            if not response.data:
                ...
                with col1:
                    selected_actions = st.multiselect("Filter by Action:", 
                                                      options=sorted(log_df['action'].unique()), 
                                                      default=log_df['action'].unique())
                with col2:
                    chart_type = st.radio("Choose Layout:", ["Bar Chart", "Line Chart"], horizontal=True)

                filtered_df = log_df[log_df['action'].isin(selected_actions)]
                chart_data = filtered_df['action'].value_counts().reset_index()
                ...
                if chart_type == "Bar Chart":
                    ...
                    chart = alt.Chart(chart_data).mark_line(color='#2E8B57', point=True).encode(
                        x='Action',
                        y='Count',
                        tooltip=['Action', 'Count']
                    )
                
                st.altair_chart(chart.properties(title="Customized Activity View"), use_container_width=True)
        except Exception as e:
